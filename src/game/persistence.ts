...
export function importSavedGame(data: SavedGameData): SavedGameSummary {
  if (!data || typeof data !== 'object') throw new Error('invalid replay JSON');
  if (!data.initialConfig || !Array.isArray(data.moves) || !Array.isArray(data.finishedPlayers)) {
    throw new Error('replay JSON missing required fields');
  }

  // Avoid clobbering an existing replay with the same id.
  let id = data.id || `imported-${Date.now()}`;
  const existing = new Set(getSavedGamesList().map((g) => g.id));
  if (existing.has(id)) id = `${id}-imported-${Date.now()}`;

  // Restore s coordinates (mirrors loadSavedGame).
  const moves: Move[] = data.moves.map((m) => ({
    ...m,
  ...
  const dateSaved = Date.now();
  const normalizedMoves = normalizeMoveHistory(moves, data.initialConfig.activePlayers);
  const tempSavedGame = {
    id,
  ...
  const winner: PlayerIndex = data.finishedPlayers[0]?.player
    ?? (data.initialConfig.activePlayers[0] as PlayerIndex);
  const summary: SavedGameSummary = {
    id,
    dateSaved,
    playerCount: data.initialConfig.playerCount,
    activePlayers: [...data.initialConfig.activePlayers],
    winner,
    totalMoves: normalizedMoves.length,
    totalTurns: Math.max(1, normalizedMoves[normalizedMoves.length - 1]?.turnNumber ?? 1),
    longestHop: longestHopGain,
    ...(data.initialConfig.playerColors ? { playerColors: { ...data.initialConfig.playerColors } } : {}),
    ...(data.initialConfig.aiPlayers ? { aiPlayers: { ...data.initialConfig.aiPlayers } } : {}),
    ...(data.initialConfig.teamMode ? { teamMode: true } : {}),
  };
  ...
  const gameData: SavedGameData = {
    ...
    id,
    moves: normalizedMoves.map(serializeMove),
    finishedPlayers: data.finishedPlayers.map((fp) => ({ ...fp })),
    dateSaved,
  };

  try {
    localStorage.setItem(GAME_DATA_PREFIX + id, JSON.stringify(gameData));
  } catch {
    throw new Error('failed to write replay to localStorage');
  }
